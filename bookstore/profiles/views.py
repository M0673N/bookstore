...
class ProfileView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        is_liked_by_user = author.authorlike_set.filter(
            user_id=self.request.user.id
        ).exists()
        is_disliked_by_user = author.authordislike_set.filter(
            user_id=self.request.user.id
        ).exists()
        has_reviewed = author.authorreview_set.filter(
            user_id=self.request.user.id
        ).exists()
...
class LikeAuthorView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        liked_by_user = author.authorlike_set.filter(
            user_id=self.request.user.id
        ).first()
        disliked_by_user = author.authordislike_set.filter(
            user_id=self.request.user.id
        ).first()
        if liked_by_user:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            message = render_to_string(
                "profile/contact_author_email.html",
                {
            ...
            return render(
                self.request, "profile/send_author_a_message.html", {"form": form}
            )
